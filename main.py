    ...
    for config in array_configuration:
        try:
            if config.startswith('ss'):
                ...
                non_title_config = f"SS-{ip}:{port}-id"
            ...
            if config.startswith('trojan'):
                ...
                non_title_config = f"TR-{ip}:{port}-id"
                country_config_dict[non_title_config] = config
            ...
            if config.startswith('vless'):
                ...
                non_title_config = f"VL-{ip}:{port}-id"
                country_config_dict[non_title_config] = config
            ...
            if config.startswith('vmess'):
                ...
                dict_params['ps'] = ''
                dict_params = {k: v for k, v in sorted(dict_params.items(), key=lambda item: item[0])}
                non_title_config = f"VM-{ip}:{port}-id"
            ...
            if config.startswith('tuic'):
                ...
                non_title_config = f"TUIC-{ip}:{port}-password"
                country_config_dict[non_title_config] = config
            ...
            if config.startswith('hy2'):
                ...
                non_title_config = f"HYSTERIA2-{ip}:{port}-password"
                country_config_dict[non_title_config] = config
...
adv_bool = False
adv_sign = "\U000026A0\U0000FE0F UNDER CONSTRUCTION \U0001F534 REPORT TECHNICAL ISSUES"
# Define develooper based on protocol type
